...
int main(int argc, char **argv) {
    ...
    double speed = 0.003; //speed is the time increment in seconds between each iteration
    ...
    while (true)
    {
        ...
        model_state_srv_msg.request.model_state.pose.position.y = circle_radius * sin(time*time*speed);
        model_state_srv_msg.request.model_state.pose.position.z = 1 + circle_radius * cos(time*time*speed);
